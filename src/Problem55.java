...
public class Problem55 {
    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < 10000; i++)
            if (isLychrelNumber(new BigInteger(String.valueOf(i)), 0))
                sum++;
        System.out.println(sum);
    }

    public static boolean isLychrelNumber(BigInteger n, int count) {
        count++;
        if (count >= 50) return true;
        BigInteger number = n.add(new BigInteger(EulerUtils.reverse(n.toString())));
        if (EulerUtils.isPalindrome(number.toString())) return false;
        return isLychrelNumber(number, count);
    }
}
